        ...
        if backend_path.is_dir():
            ...
            if manifest_path.exists():
                with open(manifest_path) as f:
                    ...
                    table.add_row(
                        manifest.get("name", backend_path.name),
                        manifest.get("version", "N/A"),
                        ", ".join(manifest.get("supported_elements", []))
                    )
...
def resolve_backend(args: argparse.Namespace, config_data: dict) -> str:
    return args.backend or config_data.get("config", {}).get("backend", "html")

def build(args: argparse.Namespace) -> int:
    config_path = find_config()
    if not config_path:
        console.print("[bold red]Error:[/bold red] No config file found.")
        return 1
    ...
    try:
        ...
        with console.status(f"[bold green]Generating {backend} backend..."):
            render_app(tree, backend, output_dir=output_dir, config_data=config_data)
        
        console.print(f"[bold green]✔ Done![/bold green] Generated output using [bold]{backend}[/bold].")
...
def main() -> int:
    ...
    parser.add_argument("-b", "--backend", type=str, help="Backend to use.")
    parser.add_argument("-w", "--watch", action="store_true", help="Watch config file.")

    subparsers = parser.add_subparsers(dest="command")
    subparsers.add_parser("init", help="Create default stencil.yaml.")
    subparsers.add_parser("list", help="List available backends.")
